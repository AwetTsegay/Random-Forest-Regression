...
# Applying random forest method to the Boston dataset using the `randomForest` package in R.

# Applying random forest program to the Boston data set to predict `medv`.

# For a regression problem with p attributes, we sample p/3 attributes at each node 
...
### Building a random forest of regression tree.
rf <- randomForest(medv~.,data=boston.train,mtry=m,ntree=100)
rf
plot(rf, col="red",main = "Random Forest Plot")

### Computing the test Mean squared error (MSE).
...
### Computing the train Mean squared error (MSE).
...
### Next, I want to see how the number of trees affect the test MSE.
...
### Next, I want to see how the number of trees affect the train MSE.
